...
uniform float iTime;uniform float iFreq0;uniform float iGreyScale;
...
void main() {
 	...
 	//vec2 uv = 0.5 * vec2( (gl_FragCoord.x / iResolution.x)*1.2-0.6, (gl_FragCoord.y / iResolution.y)*-2.4+0.8 );
	...
	if (iGreyScale > 0.0) { rgb = greyScale( rgb ); }
